...
Router.get('/', async (req, res) => {
    const options = {}
    try {
        const tyresNew = await db.TyresNew.findAll(options)
        res.json(tyresNew)
    }catch(error) {
        console.log(error)
...
Router.get('/single/:id', async (req, res) => {
    try {
        const newTyres = await db.TyresNew.findByPk(req.params.id)
        res.json(newTyres)
    } catch(error) {
        console.log(error)
...
Router.post('/new', async (req, res) => {
    try {
        await db.TyresNew.create(req.body)
...
Router.put('/edit/:id', async (req, res) => {
    try{ 
        const newTyres = await db.TyresNew.findByPk(req.params.id)
        await newTyres.update(req.body) 
