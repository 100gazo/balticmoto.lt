...
const AdminPannelTyresUsed = () => {
    const [part, setPart] = useState([])
    const navigate = useNavigate()
    const { alert, setAlert } = useContext(MainContext)

    const handleDelete = (id) => {
        axios.delete('/api/used-tyres/delete/' + id)
        .then(resp => {
            // setAlert({
            //     message: resp.data,
            //     status: 'success'
    ...
    return (
        <>
            <div className="d-flex justify-content-between page-heading">
                <h1>Devėtos Padangos</h1>
                <Link 
                    to="/admin/used-tyres/new" 
                            ...
                            <tr key={part.id}>
                                ...
                                <td>
                                    <div className="d-flex justify-content-end gap-2">
                                        <Link to={'/admin/used-tyres/edit/' + part.id} className="btn btn-primary">Redaguoti</Link>
...
export default AdminPannelTyresUsed