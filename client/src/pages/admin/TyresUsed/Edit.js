...
const EditUsedTyres = () => {
    ...
    const handleSubmit= (e) => {
        ...
        axios.put('/api/used-tyres/edit/' + id, form)
            .then(resp => {
                console.log(resp);
                navigate('/admin/used-tyres')
            }).catch(error => {
                console.log(error)
                
                if (error.response.status === 401)
                    navigate('/admin/used-tyre')
            })
        }

    useEffect(() => {
        axios.get('/api/used-tyres/single/' + id)
