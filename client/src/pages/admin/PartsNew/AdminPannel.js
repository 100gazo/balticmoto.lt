...
const AdminPannelParts = () => {
    const [part, setPart] = useState([])
    const navigate = useNavigate()
    const { alert, setAlert } = useContext(MainContext)

    const handleDelete = (id) => {
        axios.delete('/api/new-parts/delete/' + id)
        .then(resp => {
            // setAlert({
            //     message: resp.data,
            //     status: 'success'
                            ...
                            <tr key={part.id}>
                                ...
                                <td>
                                    <div className="d-flex justify-content-end gap-2">
                                        <Link to={'/admin/new-parts/edit/' + part.id} className="btn btn-primary">Redaguoti</Link>
...
export default AdminPannelParts